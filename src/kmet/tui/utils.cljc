...
(def ^:private wide-emoji-ranges
  "Codepoints terminals render as 2-column glyphs BARE: the default-emoji-
   presentation emoji (Emoji_Presentation=Yes) plus the East Asian Wide/
   Fullwidth members of the symbol and emoji blocks (pi: rgiEmojiRegex +
   eastAsianWidth; get-east-asian-width counts ambiguous as narrow). Unicode
   15.1 emoji-data.txt + EastAsianWidth.txt. Text-presentation forms (✔ ☀ ☑
   ❤ …) are NOT here — they render narrow bare and turn 2-wide only with
   U+FE0F (see VS16-EMOJI-BASES)."
  [[0x231A 0x231B] [0x2329 0x232A] [0x23E9 0x23EC] [0x23F0 0x23F0]
   ...
   [0x2B50 0x2B50] [0x2B55 0x2B55] [0x1F004 0x1F004] [0x1F0CF 0x1F0CF]
   [0x1F18E 0x1F18E] [0x1F191 0x1F19A] [0x1F1E6 0x1F202] [0x1F210 0x1F23B]
...
(def ^:private vs16-emoji-bases
  "Text-presentation RGI emoji bases (Emoji=Yes, Emoji_Presentation=No) that
   stay narrow bare (© ™ ✔ ☀ ☑ ❤ …) but render 2-column with U+FE0F (©️ ™️
   ✔️ ☀️ ☑️ ❤️ …, pi: RGI_Emoji qualified forms). Keycap bases (# * 0-9)
   are excluded — their width comes from the enclosing U+20E3, which
   char-width already counts (1 + 0 + 1 = 2)."
...
(def ^:private text-presentation-modifier-bases
  "Text-presentation Emoji_Modifier_Base codepoints (☝ ⛹ ✌ ✍ 🏋 🏌 🕴 🕵 🖐):
   a directly following skin-tone modifier is part of the glyph (☝🏽) and the
   cluster stays 2 columns. Default-presentation bases are all in
   WIDE-EMOJI-RANGES and get the same result through the PREV-W zeroing."
...
(defn- grapheme-width-and-next
  ...
  (let [cp (code-point-at s i)
        ...
        j (+ i len)
        vs16? (and (vs16-text-base? cp) (< j n) (= 0xFE0F (code-point-at s j)))
        w (char-width cp)
    ...
    (cond
      ...
      ;; Text-presentation modifier base + skin tone (☝🏽 ✌🏽 🏋🏽 …): one
      ;; width-2 glyph (pi: the RGI unqualified form; default-presentation
      ;; bases are wide already and the modifier is zeroed below)
      (and (contains? text-presentation-modifier-bases cp)
           (< j n)
           (<= skin-tone-start (code-point-at s j) skin-tone-end))
      [2 (+ j (codepoint-len s j))]

      ;; ZWJ chain on a 2-wide base: a default-presentation emoji, or a
      ;; text-presentation base with FE0F (the VS16 between base and ZWJ in
      ;; ❤️‍🔥 / 🏳️‍🌈 must not break the chain). Non-emoji wide chars (CJK)
      ;; never chain.
      (and (or (wide-emoji? cp) vs16?) (< j n)
