(ns kmet.libs.edit-diff
  "Edit matching, display-diff, and line-diff helpers shared by the built-in
   edit tool, its renderer, and extensions.
   The display diff format is line-numbered and intended for terminal rendering.

   The line-oriented Myers O(ND) diff (pi: the diff package's diffLines) lives
   here too: in a JVM Clojure project this would be io.github.java-diff-utils;
   kept in-house because Babashka can't run it."
